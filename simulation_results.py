...
curve = curve.dropna(subset=["Site", "Product", "Safety Stock Units", "Fill Rate"]).sort_values(["Site", "Product", "Safety Stock Units"])

# Build lookup
curve_lookup = {(site, product): grp.groupby("Safety Stock Units", as_index=False)["Fill Rate"]
                        .mean()
...
master = master[master.apply(lambda row: (row["Site"], row["Product"]) in curve_lookup, axis=1)].copy()
...
for _, row in master.iterrows():
    ...
    current_fill_rates.append(np.interp(row["SS Settings"], grp["Safety Stock Units"], grp["Fill Rate"]))
    model_fill_rates.append(np.interp(row["Units"], grp["Safety Stock Units"], grp["Fill Rate"]))

master["Current Fill Rate"] = current_fill_rates
master["Model Fill Rate"] = model_fill_rates

output = master[["Site", "Product", "Units", "SS Settings", "Target Fill Rate", "Current Fill Rate", "Model Fill Rate"]].copy()
output["Target Fill Rate"] = output["Target Fill Rate"] / 100
